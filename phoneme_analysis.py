...
#         # Ensure the directory exists in both locations
#         output_dir_ide = os.path.join("static", "audio")
#         output_dir_local = os.path.join("local_static", "audio")  # Define a local directory

#         os.makedirs(output_dir_ide, exist_ok=True)
#         os.makedirs(output_dir_local, exist_ok=True)

#         output_audio_file_ide = os.path.join(output_dir_ide, f"{sanitized_phoneme}.wav")
#         output_audio_file_local = os.path.join(output_dir_local, f"{sanitized_phoneme}.wav")

#         print(f"Generating audio for phoneme: {phoneme} at {output_audio_file_ide} and {output_audio_file_local}")

#         subprocess.run(["espeak-ng", phoneme, "-w", output_audio_file_ide], check=True)
        
#         if os.path.exists(output_audio_file_ide) and os.path.getsize(output_audio_file_ide) > 0:
#             print(f"Audio file generated: {output_audio_file_ide}")
#             # Copy the file to the local directory
#             shutil.copy(output_audio_file_ide, output_audio_file_local)
#             return output_audio_file_ide
#         else:
#             print(f"Failed to generate audio file: {output_audio_file_ide}")
#             return None
#     except subprocess.CalledProcessError as e:
#         print(f"Error generating audio for phoneme {phoneme}: {e}")
#         return None
